...
jQuery(document).ready(function () {
    jQuery('#myFile').bind('change', function () {
        sizeInBytes = this.files[0].size;
        sizeInMB = (this.files[0].size / (1024 * 1024)).toFixed(2);
        var sizeMessage = "";
        if (sizeInMB > 10) {
            alert("The file you are uploading is " + sizeInMB + " MB and will take some time to upload depending on your internet speed.  After clicking upload, stay on the page until you get a notification of it being sent");
        }
...
function addMoreFiles() {
    jQuery('.moreFiles').prev('p').remove();
    jQuery('#amf').hide();
    jQuery('.moreFiles').fadeIn();
