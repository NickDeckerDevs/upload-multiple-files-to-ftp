...
function efu_easy_ftp_upload_post() {
    ...
    for ($i = 0; $i < count($_FILES['file']['name']); $i++) {
	    $source = $_FILES['file']['tmp_name'][$i]; // retrieve name of the source file to be uploaded
	    $dest = $_FILES['file']['name'][$i];
        if ($dest != "") {
            echo 'file: '.$dest.'<br>';
            $fileNamesUploaded .= $dest.PHP_EOL;
        }
    ...
    $notifysubject = 'FTP Upload Notification '.$successCount.' Successful - '.$failCount.' Failed'; //successful upload
	...
	$notifymessage .= 'Other Notes or Instructions: '.$notes.PHP_EOL;
    $notifymessage .= 'File names uploaded: '.$fileNamesUploaded.PHP_EOL;
	$notifymessage .= 'Login to ftp(the file will be in the folder '.$company.'):    ftp://'.$ftp_user_name.':'.$ftp_user_pass.'@'.$server;
    
	
	$notifyheader = 'From: FTP Uploads <'.$email_add.'>'.PHP_EOL;
	...
	echo '<br/><p class="EFU_notify">'.$successful.'</p>';
